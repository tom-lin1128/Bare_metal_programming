...
int mailbox_call(unsigned int *mailbox){
    ...
    const int data = ((unsigned int)((unsigned long)mailbox)&~0xF) | ((char)MAILBOX_CH_PROP & 0xF);//msb 28 bits is data and lsb 4 bits is channel1
...
int mailbox_board_revision(){
     ...
     if ( mailbox_call(mailbox) )  return mailbox[5];
     else                          return 0;
...
long mailbox_arm_memory(){
    ...
    if( mailbox_call(mailbox) ) return ((mailbox[5] << 32) | mailbox[6]);
    else                        return 0;
}