...
void init_allocator(){
    ...
    for(int i = 0; i < 2; i++){
        ...
        allocate_list[i].isfull = 2;//allc frame full test
        allocate_list[i].isempty = 1;
...
void* fix_malloc(int size){
    ...
    if(tmp->next == nullptr && tmp->isfull == 2){
        ...
        free_allocate_frame[i].isempty = 0;
        ...
        //printf_i(free_allocate_frame[i].isfull);
        //printf_s(" lab1\n");
        return (void *)free_chunk[j].start_address;
    }

    //printf_s(" lab1\n");
    while((tmp->allocate_size != alloc_size || tmp->isfull == 1) && tmp->next){
        tmp = tmp->next;
    }
    //printf_s(" lab2\n");
    if(tmp->allocate_size == alloc_size){
        if(tmp->isfull == 0){
            ...
            if(chunk_tmp->isused == 0){ // exist block (error revise)
                chunk_tmp->isused = 1;
            ...
            free_allocate_frame[i].allocate_size = alloc_size;
            free_allocate_frame[i].isfull = 0;
            free_allocate_frame[i].isempty = 1;
            free_allocate_frame[i].start_address = frame_addr;
            free_allocate_frame[i].next = nullptr;
            free_allocate_frame[i].first = &free_chunk[j];
...
void *dynamic_malloc(int size){
    if(size <= 2048)
        ...
        while(tmp->next && (tmp->isfull == 1 || tmp->isfull == 2))
            tmp = tmp->next;
...
void free(void *address){
    ...
    printf_c('\n');*/
    //printf_s("lab1\n");
    while((((int)address - tmp->start_address >= 4096 || (int)address - tmp->start_address < 0) || tmp->isfull == 2) && tmp->next ){
        tmp = tmp->next;
    }
    //printf_s("lab2\n");
